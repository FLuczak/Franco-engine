...
{
    bool isLine = Editor->GetStyle().StraightLines;
    if (flags == None)
    {
        drawList->ChannelsSetCurrent(c_LinkChannel_Links);
        Draw(drawList, m_Color, isLine, 0.0f);
    }
    else if (flags & Selected)
    {
        ...
        Draw(drawList, borderColor, isLine, 4.5f);
    }
    else if (flags & Hovered)
    {
        ...
        Draw(drawList, borderColor, isLine, 2.0f);
    }
    else if (flags & Highlighted)
    {
        drawList->ChannelsSetCurrent(c_LinkChannel_Selection);

        Draw(drawList, m_HighlightColor, isLine, 3.5f);
    }
}

void ed::Link::Draw(ImDrawList* drawList, ImU32 color, bool isLine,float extraThickness) const
{
    if (!m_IsLive)
        return;

    const auto curve = isLine ? GetLine():GetCurve();
...
ImCubicBezierPoints ed::Link::GetLine() const
{
    ImCubicBezierPoints result;
    result.P0 = m_Start;
    result.P1 = ImLerp(m_Start,m_End,0.33f);
    result.P2 = ImLerp(m_Start, m_End, 0.66f);
    result.P3 = m_End;
...
void ed::EditorContext::End()
{
    ...
    for (auto link : m_Links)
        if (link->m_IsLive && link->IsVisible())
            link->Draw(m_DrawList, Object::None );
