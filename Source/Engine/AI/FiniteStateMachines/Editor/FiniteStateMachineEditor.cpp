...
AI::FiniteStateMachineEditor::FiniteStateMachineEditor()
{
...
void AI::FiniteStateMachineEditor::SelectedStateNameInputBox(const std::unique_ptr<AI::NodeWrapper>& focusedNode)
{
...
void AI::FiniteStateMachineEditor::SelectedStateTypeSelection(const std::unique_ptr<AI::NodeWrapper>& focusedNode) const
{
    ImGui::Text("State");
    ImGui::SameLine();
...
void AI::FiniteStateMachineEditor::SelectedNodeTransitionButtons(const std::unique_ptr<AI::NodeWrapper>& focusedNode, int index,int stateToGoToChoice, AI::NodeTransition& transition) const
{
...
void AI::FiniteStateMachineEditor::SelectedNodeTransitionComparatorButtons(int index, std::vector<int>& transitionsToRemove,AI::NodeTransition& transition) const
{
    ImGui::SameLine();
...
void AI::FiniteStateMachineEditor::ComparatorValueInput(ComparatorWrapper& comparator, const char* label) const
{
...
void AI::FiniteStateMachineEditor::ChooseComparatorTypeInput(AI::ComparatorWrapper& comparator, const char* label) const
{
...
void AI::FiniteStateMachineEditor::ChooseComparisonTypeInput(AI::ComparatorWrapper& comparator, const char* label) const
{
...
void AI::FiniteStateMachineEditor::DrawTransitions(EditorFSMWrapper& wrapper) const
{
    int index = 0;
    for (const auto& element : wrapper.transitions)
    {
        ...
        {
            if ((ax::NodeEditor::GetNodePosition(element.first).x -ax::NodeEditor::GetNodePosition(transition.nodeTo).x) > 0)
            {
...
void AI::FiniteStateMachineEditor::DeleteNode(size_t nodeIdToRemove)
{
    m_selectedNodes.clear();

    erase_if(m_editorFSM.nodes, [nodeIdToRemove](std::unique_ptr<NodeWrapper>& node) { return node->id == nodeIdToRemove; });

    m_editorFSM.transitions.erase(nodeIdToRemove);

    for (auto& element : m_editorFSM.transitions)
    {
        erase_if(element.second, [nodeIdToRemove](const NodeTransition& t) { return nodeIdToRemove == t.nodeTo; });
    }

    m_removedIds.push_back(nodeIdToRemove);
}

void AI::FiniteStateMachineEditor::ResetHoverInfo() { m_hovered = -1; }

void AI::FiniteStateMachineEditor::DrawNode(const AI::EditorFSMWrapper& wrapper, size_t i)
{
...
void AI::FiniteStateMachineEditor::DrawNodes(const EditorFSMWrapper& wrapper)
{
...
void AI::FiniteStateMachineEditor::DrawNodes(const EditorFSMWrapper& wrapper, const StateMachineContext& context)
{
    ax::NodeEditor::PushStyleVar(ax::NodeEditor::StyleVar_PinArrowSize, 50.0f);
    ax::NodeEditor::PushStyleVar(ax::NodeEditor::StyleVar_PinArrowWidth, 10.0f);
...
std::vector<std::string> AI::FiniteStateMachineEditor::GetStateNames() const
{
...
void AI::FiniteStateMachineEditor::DrawUpperMenu()
{
    ...
    {
        const size_t size = m_editorFSM.nodes.size() + 1;
...
void AI::FiniteStateMachineEditor::SaveStates(nlohmann::json& states) const
{
    if (m_editorFSM.nodes.empty()) return;
...
void AI::FiniteStateMachineEditor::SaveTransitions(nlohmann::json& json) const
{
...
void AI::FiniteStateMachineEditor::HandleClicks()
{
...
void AI::FiniteStateMachineEditor::DrawPopupMenu()
{
...
void AI::FiniteStateMachineEditor::SaveEditorData(nlohmann::json& json) const
{
...
void AI::FiniteStateMachineEditor::Update()
{
    auto& io = ImGui::GetIO();
...
void AI::FiniteStateMachineEditor::PreviewContext(const FiniteStateMachine& fsm, const StateMachineContext& context)
{
    auto& io = ImGui::GetIO();
    bool open = true;
...
void AI::FiniteStateMachineEditor::SaveFSM() const
{
    ...
    {
        outputFile << std::setw(save.size()) << save << std::endl;
        outputFile.close();
    }
    else
    {
...
size_t AI::FiniteStateMachineEditor::GetIdToSet()
{
...
void AI::FiniteStateMachineEditor::CreateNode()
{
...
void AI::FiniteStateMachineEditor::LoadStates(nlohmann::json& states, EditorFSMWrapper& wrapper)
{
    for (size_t i = 0; i < states.size(); i++)
    {
        ...
        std::erase_if(temp->stateType, isspace);
        temp->underlyingState = std::move(GenericFactory<State>::Instance().CreateProduct(temp->stateType));

        for (auto element : states[i]["editor-variables"])
        {
            auto name = element["name"].get<std::string>();
            if (!temp->underlyingState->editorVariables.contains(name)) continue;
...
void AI::FiniteStateMachineEditor::DeserializeComparators(nlohmann::json& comparators, NodeTransition& node_transition)
{
...
void AI::FiniteStateMachineEditor::LoadTransitions(nlohmann::json& transitionData, EditorFSMWrapper& wrapper)
{
...
void AI::FiniteStateMachineEditor::LoadEditorData(const nlohmann::json& json, EditorFSMWrapper& wrapper)
{
...
void AI::FiniteStateMachineEditor::LoadFSM()
{
